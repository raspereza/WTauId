...
void make2D_mtgen_genTauWPt_Reweighting() {
  ...
  for(unsigned int idx_iso=0; idx_iso<iso.size(); idx_iso++){
	...
	makeSelection(dir+"/"+samples[i].second[idx_list]+".root","NTuple",getXSec(samples[i].second[idx_list]),iso[idx_iso],select,histo,var1,var1,var2);
    ...
    canv->Print("figures/NumberOfEvents_" + var1 + "_" + var2 + "_WToTauNu_" + iso[idx_iso] + ".pdf");

    // Save histogram in file
    TFile *out = new TFile("output/ReweightingWeights_" + var1 + "_" + var2 + "_WToTauNu" = iso[idx_iso] + ".root","RECREATE");
    out->cd();
